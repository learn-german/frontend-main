...
### Dữ liệu
- Bảng mới `reading_passages`: `id` (uuid), `lesson_id` (FK `lessons`), `text_de` (text, tiếng Đức — **không có bản dịch tiếng Việt**), `order_index` (int, thứ tự = thứ tự tạo, không hỗ trợ kéo-thả).
...
- Migration backfill: với mỗi bài học đang có `reading_text` (ví dụ `a1-l1`), tạo 1 dòng `reading_passages` từ `reading_text` (`order_index=0`, bỏ qua `reading_text_vi` vì không còn dùng bản dịch), rồi gán `reading_passage_id` cho toàn bộ câu hỏi `doc` hiện có của bài đó vào đoạn mới này — không mất dữ liệu đã soạn trước đó.
...
- Tab "Đọc": mỗi bài học hiện thêm danh sách các đoạn văn đã soạn (mỗi đoạn: 1 ô nhập tiếng Đức duy nhất, nút xóa, số câu hỏi thuộc đoạn đó).
- Nút "Thêm đoạn văn mới" cho mỗi bài học → tạo 1 dòng `reading_passages` mới (textarea trống). Lưu bằng nút "Lưu" riêng cho từng đoạn văn (hiện khi có thay đổi chưa lưu) — pattern đơn giản, khớp với các form khác trong trang.
- "Thêm câu hỏi" trong tab Đọc giờ yêu cầu chọn thuộc đoạn văn nào trước khi mở form câu hỏi — câu hỏi lưu kèm `reading_passage_id`.
...
- Tab "Đọc" trên trang bài học: hiện danh sách các đoạn văn (chỉ tiếng Đức), thay vì 1 đoạn duy nhất như hiện tại.
...
### `completion.ts`
- `applicableCategories`: điều kiện "Đọc áp dụng" đổi từ `readingText` (string) sang "bài học có ít nhất 1 `reading_passages`". Cần đổi field trên `Lesson`/`LessonContentFlags` phản ánh việc này (ví dụ `readingPassages: {id, textDe}[]` thay cho `readingText`/`readingTextVi`).
...
- Không có bản dịch tiếng Việt cho đoạn văn Đọc (chỉ tiếng Đức) — `reading_text_vi` không được đọc ở bất kỳ đâu trong tính năng mới này.

## Testing / verification

- `npm run lint` pass.
- Migration: verify live DB — bài `a1-l1` có đúng 1 `reading_passages` row từ `reading_text` cũ, câu hỏi Đọc hiện có được gán đúng `reading_passage_id`; các bài khác không có `reading_text` thì không tạo đoạn nào.
