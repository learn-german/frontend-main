...
### Bảo mật đáp án
`question_text` (bảng gốc) chứa đáp án thật trong `{{...}}`, nên **không được** gửi nguyên văn cho client. `quiz_questions_public` (view công khai, admin/client-facing hooks đọc từ đây) sẽ thay mỗi `{{...}}` bằng token cố định `{{blank}}` ngay trong định nghĩa view (SQL `regexp_replace`). Học viên chỉ thấy `Ich {{blank}} Student. Ich komme {{blank}} Vietnam.` — không bao giờ thấy đáp án qua network response.

**Quan trọng — việc ẩn `{{...}}` áp dụng vô điều kiện cho MỌI row trong view, không phụ thuộc cột `type`.** Không viết `CASE WHEN type = 'fill-blank' THEN regexp_replace(...) ELSE question_text END` — vì nếu admin lỡ đổi `type` của 1 câu hỏi sang loại khác (multiple-choice, matching...) mà quên xoá `{{đáp_án}}` còn sót lại trong `question_text`, cách viết có điều kiện theo `type` sẽ khiến đáp án lộ nguyên văn qua view công khai. Áp `regexp_replace` lên cột `question_text` của **tất cả** các row bất kể `type` loại bỏ hoàn toàn rủi ro này — vì cú pháp `{{...}}` gần như không bao giờ xuất hiện tình cờ trong văn bản câu hỏi các loại khác, nên áp dụng vô điều kiện không ảnh hưởng gì đến chúng, chỉ thêm 1 lớp an toàn.

Edge Function `quiz-submit` (chạy `service_role`, đã đọc trực tiếp từ bảng gốc `quiz_questions` chứ không qua view) tự tách các đáp án từ `{{...}}` trong `question_text` gốc để chấm — dùng regex giống nhau ở cả 2 nơi (SQL view và Edge Function) để đảm bảo nhất quán vị trí/số lượng ô trống. Việc quyết định "câu hỏi này có phải multi-blank không" dựa vào **sự có mặt của `{{...}}` trong `question_text`**, không dựa vào `type`, để nhất quán với cách view ẩn đáp án (cũng không dựa vào `type`).
...
- **Trường hợp đổi nhầm `type`:** tạo 1 câu hỏi với `question_text` chứa `{{đáp_án}}` nhưng `type` đặt là `multiple-choice` (không phải `fill-blank`) → verify view `quiz_questions_public` vẫn ẩn `{{đáp_án}}` thành `{{blank}}`, không lộ nguyên văn, xác nhận việc ẩn không phụ thuộc `type`.
