# Phase 4 (UI admin) — Viết lại AdminQuizSection theo set — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `AdminQuizSection.tsx` viết lại hoàn toàn — quản lý câu hỏi Nghe/Đọc qua `grammar_exercises`/`exercise_sets` (set_id), không còn phụ thuộc bảng `quiz_questions` đã xoá. Giữ nguyên CRUD `listening_clips`/`reading_passages`.

**Architecture:** 1 file `src/pages/admin/AdminQuizSection.tsx` viết lại hoàn toàn (không tạo file mới — cùng tên, cùng vị trí, `AdminPage.tsx` không cần đổi import). Cấu trúc: quản lý clip/đoạn văn (khu vực riêng, luôn hiện) → danh sách set (`useExerciseSets`) → trong mỗi set: chọn clip/đoạn văn (chỉ khi set chưa có câu hỏi) + bảng câu hỏi + modal thêm/sửa 3 loại câu.

**Tech Stack:** React 19 + TypeScript — không có gì mới, tái dùng `useExerciseSets`/`useModuleOrder`/`AdminModuleGroup`/`uploadMedia`/`useMediaPlaybackUrl`/`quizAnswerCodec`.

## Global Constraints

- Không đổi `useExerciseSets.ts`, `useMediaPlaybackUrl.ts`, `uploadMedia.ts`, `AdminModuleGroup.tsx`, `useModuleOrder.ts`, `AdminGrammarExerciseSection.tsx` — chỉ tái dùng.
- Không thêm cột DB mới. Liên kết set↔clip/đoạn văn suy ra từ `audio_clip_id`/`reading_passage_id` của **câu hỏi đầu tiên** trong set (không cần cột riêng).
- Quản lý clip/đoạn văn (upload/xoá/sửa) là khu vực **độc lập, luôn hiện** trước danh sách set trong mỗi lesson — không gộp vào modal thêm câu hỏi (đơn giản hơn spec ban đầu mô tả, cùng tinh thần: chọn từ danh sách có sẵn khi tạo set đầu tiên).
- `matching_pairs`/`correct_answer` của type `matching`: admin chỉ nhập cặp de/vi, `correct_answer` tự sinh qua `serializeMatching` (đã có, không viết lại).

---

### Task 1: Viết lại `AdminQuizSection.tsx` — phần dữ liệu + clip/đoạn văn (giữ nguyên)

**Files:**
- Modify: `src/pages/admin/AdminQuizSection.tsx` (ghi đè toàn bộ nội dung)

**Interfaces:**
- Produces: `export const AdminQuizSection: React.FC` — không đổi cách gọi từ `AdminPage.tsx` (`<AdminQuizSection />`, không props).

- [ ] **Step 1: Ghi đè phần đầu file — imports, types, fetch dữ liệu**
...
export const AdminQuizSection: React.FC = () => {
  ...
  const fetchData = async () => {
    const setIdsKey = quizSetIds.join(",");
...
- [ ] **Step 2: Giữ nguyên CRUD clip/đoạn văn (copy y hệt bản cũ, chỉ đổi `fetchQuestions()` → `fetchData()`)**
...
- [ ] **Step 3: `npm run lint` — kỳ vọng lỗi (JSX/return chưa viết, các state Task 2-4 chưa khai báo dùng tới) — bỏ qua, tiếp tục Task 2-4 trước khi build lại.**

Không commit ở task này (file chưa compile được) — commit gộp ở cuối Task 4.

---

### Task 2: Set list + form thêm/sửa câu hỏi (state + handlers)

**Files:**
- Modify: `src/pages/admin/AdminQuizSection.tsx` (nối tiếp Task 1, cùng file)

- [ ] **Step 1: Thêm state cho modal thêm/sửa câu hỏi + set, và các helper suy ra clip/đoạn văn của 1 set**
...
```tsx
  ...
  const [editExerciseId, setEditExerciseId] = useState<string | null>(null); // null = tạo mới
  const [modalLessonId, setModalLessonId] = useState<string>("");
  const [modalSetId, setModalSetId] = useState<string | null>(null); // null = tạo set mới cùng lúc
...
- [ ] **Step 2: `handleSave` — tạo set (nếu cần) rồi insert/update câu hỏi**
...
- [ ] **Step 3: Chưa chạy lint — JSX vẫn ở Task 3-4. Tiếp tục.**

---

### Task 3: JSX — clip/đoạn văn area + danh sách set + bảng câu hỏi

**Files:**
- Modify: `src/pages/admin/AdminQuizSection.tsx` (nối tiếp Task 2)

- [ ] **Step 1: Component con `QuestionTable` + `ClipManageRow`/`PassageManageRow` (khu vực quản lý media, luôn hiện)**
...
- [ ] **Step 2: `render` — đầu hàm (trước `return`), gộp lesson với module + search**
...
**LƯU Ý:** `useState` không được khai báo sau các biến `const` tính toán từ state khác trong cùng component — di chuyển 3 dòng `useState` ở Step 2 lên **cùng chỗ khai báo state ở Task 2 Step 1** (đầu component, trước `getSetMediaId`), chỉ giữ `lessonGroups`/`filteredGroups`/`moduleSections` ở vị trí ngay trước `return`.

- [ ] **Step 3: JSX chính — thay toàn bộ phần `return (...)` cũ**
...
**LƯU Ý quan trọng:** `PassageEditRow` chưa tồn tại — thêm component này ở Step 1 (Task 3), cạnh `ClipRow`, y hệt phần textarea+nút Lưu/Xoá của `PassageCard` cũ (dòng ~190-225 bản gốc) nhưng bỏ phần "+ Câu hỏi" (đã chuyển vào set):
...
### Task 4: Modal thêm/sửa câu hỏi + dialog xác nhận xoá + wiring

**Files:**
- Modify: `src/pages/admin/AdminQuizSection.tsx` (nối tiếp Task 3, phần cuối `return`, trước `</div>` đóng ngoài cùng)

- [ ] **Step 1: Modal thêm/sửa câu hỏi**
...
- [ ] **Step 2: Dialog xác nhận xoá (câu hỏi/clip/đoạn văn) — copy y hệt bản cũ, đổi `deleteTarget.question_text` → `deleteTarget.prompt_text`, đổi `handleDelete`/`handleDeleteClip`/`handleDeletePassage` cho đúng state mới**
...
- [ ] **Step 3: `npm run lint`, sửa lỗi cho tới khi sạch**

Các lỗi dễ gặp và cách sửa:
- `useState` khai báo sau khi dùng (JSX Step 2 Task 3 note) → di chuyển toàn bộ `useState` lên đầu component, cùng khối state Task 2 Step 1.
- Biến `EditForm` dùng trong `setForm((prev) => ({ ...prev, type: e.target.value as EditForm["type"] }))` — đảm bảo type `EditForm` được export/định nghĩa trước khi dùng (đã khai báo ở Task 2 Step 1, cùng file, nằm trước — không cần export vì chỉ dùng nội bộ).
- `moduleSections`/`filteredGroups`/`lessonGroups` dùng biến `activeTab` — đã có ở đầu component, không vấn đề.

- [ ] **Step 4: Commit**

```bash
git add src/pages/admin/AdminQuizSection.tsx
git commit -m "feat(admin): viết lại AdminQuizSection theo set — quản lý Nghe/Đọc qua grammar_exercises"
```
...
```
Kỳ vọng: pass toàn bộ, không có test nào tham chiếu `AdminQuizSection.tsx` cũ (kiểm tra bằng `grep -rn "AdminQuizSection" src/**/*.test.*` trước khi kết luận).
...
- [ ] **Step 4: Test tay (sau khi build sạch)**

Mở `/admin` → Bài tập → tab Nghe: tải 1 file mp3 test → "+ Thêm bộ bài tập" → chọn file mp3 vừa tải → thêm 1 câu `multiple_choice` → publish set → mở app học viên, vào lesson đó, tab Nghe, xác nhận: set hiện đúng, audio phát được, chọn đáp án, nộp bài, chấm điểm đúng. Lặp lại nhanh cho tab Đọc với 1 đoạn văn + 1 câu `text_fill_blank` hoặc `matching`.
