...
Chia 3 phase độc lập, xem spec — cả 3 phase đã xong:
...
- [x] Phase 3a — Admin dùng chung form tạo/sửa câu hỏi: [2026-08-06-admin-shared-exercise-form-design.md](docs/superpowers/specs/2026-08-06-admin-shared-exercise-form-design.md). Đã xong: `AdminGrammarExerciseSection.tsx` giờ nhận prop `category`, hỗ trợ đủ 10 loại câu hỏi (port `text_fill_blank`/`matching` từ form cũ của `AdminQuizSection.tsx`) kèm group/hint/word-bank/kéo-thả, tự quản lý clip mp3/đoạn văn khi category khác `nguphap`. `AdminQuizSection.tsx` giờ chỉ còn tab switcher, mount component chung cho cả 3 category — xoá ~700 dòng form/table riêng. Sửa luôn bug phát hiện trong lúc làm: tab Ngữ pháp trước đây fetch toàn bộ `grammar_exercises` không lọc category, câu hỏi nghe/đọc bị hiện lẫn vào. 131/131 test pass, lint sạch — **chưa verify được trên trình duyệt thật** (sandbox không có `.env.local`), cần tự test theo Task 7 trong [plan](docs/superpowers/plans/2026-08-06-admin-shared-exercise-form.md).
- [x] Phase 3b — học viên nhìn thấy group/hint/word-bank khi làm bài nghe/đọc: [2026-08-06-quiz-group-hint-wordbank-design.md](docs/superpowers/specs/2026-08-06-quiz-group-hint-wordbank-design.md). Đã xong: port nguyên logic grouping/hint/word-bank từ `GrammarExerciseSetBody` sang `QuizExerciseSetBody` (chỉ đổi `QuizSetListPage.tsx` + 1 export mới ở `GrammarExercisePage.tsx`, không đụng hành vi ngữ pháp), accordion "Bài N" theo group, hint + word-bank chip picker dùng chung, banner audio/đoạn văn giữ nguyên vị trí set-level. Sửa luôn 3 gap card kết quả chấp nhận tạm ở Phase 2 (submittedText/userGroups/classificationResults hardcode rỗng dù dữ liệu đã có sẵn) + 1 bug phát hiện thêm lúc port (blankValues không rẽ nhánh theo type, sai cho `fill_in_the_blank`). 131/131 test pass, lint sạch — **chưa verify được trên trình duyệt thật** (sandbox không có `.env.local`), cần tự test theo Task 5 trong [plan](docs/superpowers/plans/2026-08-06-quiz-group-hint-wordbank.md).
---
[Feature][Admin][Người dùng] Thêm filter cho từng cột trong bảng danh sách user — [x] Đã xong (2026-08-06)

[2026-08-06-admin-users-filter-pagination-design.md](docs/superpowers/specs/2026-08-06-admin-users-filter-pagination-design.md). Đã xong: hàm lọc thuần `filterUsers` (`src/lib/adminUserFilter.ts`, có test riêng — bắt được 1 bug thật lúc viết test: mốc `dateTo` bị parse theo giờ local thay vì UTC, làm lệch biên khoảng ngày) dùng cho filter Role/Cấp độ mở (chọn nhiều, khớp OR)/Ngày tạo, kết hợp AND với ô tìm kiếm cũ. Làm thêm ngoài spec gốc theo yêu cầu giữa chừng: phân trang 15 user/trang, tự về trang 1 khi đổi filter/search. 137/137 test pass, lint sạch — **chưa verify được trên trình duyệt thật** (sandbox không có `.env.local`), cần tự test theo Task 4 trong [plan](docs/superpowers/plans/2026-08-06-admin-users-filter-pagination.md).
